...
// Returns an integer in [3, 10]. Falls back to 5 if the model is unresponsive.
const DURATION_MODEL = 'google/gemini-flash-1.5';

async function suggestDuration(imagePath, prompt) {
    ...
    messages: [
      {
        ...
        content: [
          ...
          {
            type: 'text',
            text: `This still image will be animated into a short video clip.\n\nAction to animate: "${prompt}"\n\nHow many seconds should this clip be to complete the described action naturally at a comfortable pace? Simple gestures = shorter, multi-step sequences = longer.\n\nReply with a single integer between 3 and 10. No explanation.`,
          },
        ],
      },
    ],
  ...
  return Number.isFinite(seconds) ? Math.min(10, Math.max(3, seconds)) : 5;
}
