...
// Checks <basename>.txt then <basename>.json ({ "prompt": "..." }).
// Returns { prompt, source } where source is 'image' | 'default'.
async function loadImagePrompt(imagePath, defaultPrompt) {
  ...
  if (await fs.pathExists(txtPath)) {
    ...
    if (text) return { prompt: text, source: 'image' };
  }
...
async function processImages(options) {
  ...
  for (const imagePath of images) {
    ...
    const { prompt, source: promptSource } = await loadImagePrompt(imagePath, defaultPrompt);
    const promptTag = promptSource === 'image' ? chalk.cyan(' [custom prompt]') : '';

    if (dryRun) {
      console.log(chalk.cyan(`  would process  ${name}${promptTag}`));
      continue;
    }
    ...
    for (let i = 1; i <= count; i++) {
      ...
      const spinner = ora(`  ${label}${promptTag}`).start();

      try {
        spinner.text = `  ${label}${promptTag} — submitting`;
        ...
        spinner.text = `  ${label}${promptTag} — waiting for generation…`;
        ...
        spinner.text = `  ${label}${promptTag} — downloading`;
        await downloadVideo(videoUrl, outFile);

        outputs.push(path.basename(outFile));
        spinner.succeed(chalk.green(`  ${label}${promptTag} → ${path.basename(outFile)}`));
      } catch (err) {
