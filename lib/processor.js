...
const { submitGeneration, pollGeneration, downloadVideo } = require('./api');
...
async function processImages(options) {
  ...
  const model = options.model || 'runway/gen-3-alpha-turbo';
  ...
  for (const imagePath of images) {
    ...
    for (let i = 1; i <= count; i++) {
      ...
      try {
        ...
        const submission = await submitGeneration({ imagePath, model, prompt, resolution, duration });
