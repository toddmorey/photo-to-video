...
const { submitGeneration, pollGeneration, downloadVideo, getModelParams, suggestDuration } = require('./api');
  ...
  for (const imagePath of images) {
    ...
    if (rawDuration === 'auto') {
      if (dryRun) {
        ...
        try {
          effectiveDuration = await suggestDuration(imagePath, prompt);
    ...
    if (dryRun) {
      console.log(chalk.cyan(`  would process  ${name}${promptTag}${durationTag}`));
      continue;
    }
    ...
    for (let i = 1; i <= count; i++) {
      ...
      const spinner = ora(`  ${label}${promptTag}${durationTag}`).start();

      try {
        spinner.text = `  ${label}${promptTag}${durationTag} — submitting`;
        const submission = await submitGeneration({ imagePath, model, prompt, resolution, duration: effectiveDuration, allowedParams });
        const genId = submission.id || submission.data?.id;

        if (!genId) throw new Error('No generation ID returned from API');

        spinner.text = `  ${label}${promptTag}${durationTag} — waiting for generation…`;
        ...
        spinner.text = `  ${label}${promptTag}${durationTag} — downloading`;
        await downloadVideo(videoUrl, outFile);

        outputs.push(path.basename(outFile));
        spinner.succeed(chalk.green(`  ${label}${promptTag}${durationTag} → ${path.basename(outFile)}`));
      } catch (err) {
